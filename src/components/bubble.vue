...
<template>
  <div class="flex">
    <div
      v-if="leftToRight" class="mr-2 h-11 w-11 cursor-pointer !bg-cover !bg-center !bg-no-repeat"
      :style="{ backgroundImage: `url(${image})` }"
      @click="emit('avatarClicked')"
    />
    <div class="flex cursor-pointer" @click="emit('bubbleClicked')">
      ...
      <div class="max-w-127.5 flex border-0.125rem border-white bg-black px-3 py-1.5 text-7 font-bold leading-7 font-neue-bit text-white">
    ...
    <div
      v-if="!leftToRight" class="ml-2 h-11 w-11 !bg-cover !bg-center !bg-no-repeat"
      :style="{ backgroundImage: `url(${image})` }"
      @click="emit('avatarClicked')"
    />
  </div>
